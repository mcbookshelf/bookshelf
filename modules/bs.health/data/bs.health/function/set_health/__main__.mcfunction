...
execute store result score @s bs.hmod run compute default integer {type:sub,left:{type:min,inputs:[{type:from_float,input:{type:mul,inputs:[{type:storage,storage:"bs.health:set_health",path:"in.points"},1000000]}},{type:score,target:{type:fixed,name:"#m"},score:"bs.ctx"}]},right:{type:score,target:{type:fixed,name:"#h"},score:"bs.ctx"}}

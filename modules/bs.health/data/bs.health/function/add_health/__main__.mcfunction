...
execute store result score @s bs.hmod run compute default integer {type:min,inputs:[{type:add,inputs:[{type:score,target:"this",score:"bs.hmod"},{type:from_float,input:{type:mul,inputs:[{type:storage,storage:"bs.health:add_health",path:"in.points"},1000000]}}]},{type:sub,left:{type:score,target:{type:fixed,name:"#m"},score:"bs.ctx"},right:{type:score,target:{type:fixed,name:"#h"},score:"bs.ctx"}}]}

...
scoreboard players operation #c bs.ctx -= #a bs.ctx
scoreboard players operation #d bs.ctx -= #a bs.ctx
execute store result storage bs:ctx _._[-1].p[8] int 1 run scoreboard players operation #a bs.ctx += #c bs.ctx
scoreboard players operation #d bs.ctx += #c bs.ctx
scoreboard players operation #c bs.ctx *= -2 bs.const
scoreboard players operation #d bs.ctx -= #c bs.ctx
scoreboard players operation #u bs.ctx *= 3 bs.const
...
scoreboard players operation #c bs.ctx += #b bs.ctx
scoreboard players operation #c bs.ctx -= #d bs.ctx
execute store result storage bs:ctx _._[-1].p[9] int 1 run scoreboard players operation #b bs.ctx /= 2 bs.const
execute store result storage bs:ctx _._[-1].p[10] int 1 run scoreboard players operation #c bs.ctx /= 2 bs.const
execute store result storage bs:ctx _._[-1].p[11] int 1 run scoreboard players operation #d bs.ctx /= 2 bs.const

data remove storage bs:ctx _.points[0]

execute if data storage bs:ctx _.points[3] run function bs.animation:bake/3d/catmull_rom

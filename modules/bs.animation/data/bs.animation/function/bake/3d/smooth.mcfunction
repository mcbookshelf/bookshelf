...
execute store result score #u bs.ctx run data get storage bs:ctx _.points[-2][1] 1000
execute store result score #v bs.ctx run data get storage bs:ctx _.points[-3][1] 1000
scoreboard players operation #v bs.ctx -= #u bs.ctx
execute store result storage bs:ctx _.points[-1][1] double .001 run scoreboard players operation #u bs.ctx -= #v bs.ctx
execute store result score #u bs.ctx run data get storage bs:ctx _.points[-2][2] 1000
execute store result score #v bs.ctx run data get storage bs:ctx _.points[-3][2] 1000
scoreboard players operation #v bs.ctx -= #u bs.ctx
execute store result storage bs:ctx _.points[-1][2] double .001 run scoreboard players operation #u bs.ctx -= #v bs.ctx

function bs.animation:bake/3d/catmull_rom

...
execute if score #a bs.ctx matches 0 if score #c bs.ctx matches ..999 run return run data modify storage bs:out geometry.intersect append from storage bs:in geometry.shapes[{type:"line"}]
#check if the line is'nt included in the plane (n.v) and colinear
execute if score #a bs.ctx matches 0 if score #c bs.ctx matches 1000.. run return run function bs.geometry:error/none_intersection

#Compute the value of the parameter of the axis n.(o-o')/n.v
scoreboard players operation #c bs.ctx /= #a bs.ctx
#(which is the distance along the line from the origin to the intersection)

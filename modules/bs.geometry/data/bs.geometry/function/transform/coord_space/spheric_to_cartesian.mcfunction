...
data modify storage bs:out geometry.coord_space.origin[0] set compute default float {\
    ...
    {type:"sin",input:{type:"storage",path:'geometry.shapes[{type:"point"}].origin[1]',storage:"bs:in"}}\
    ]\
}
...
data modify storage bs:out geometry.coord_space.origin[0] set compute default float {\
    ...
    inputs:[\
    {type:"storage",path:'geometry.shapes[{type:"point"}].origin[2]',storage:"bs:in"},\
    {type:"cos",input:{type:"storage",path:'geometry.shapes[{type:"point"}].origin[1]',storage:"bs:in"}}\
    ]\
}

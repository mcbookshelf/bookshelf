...
- ⚠️ Every feature is now a number provider.
- ⚠️ Removed `#bs.bitwise:two_complement`: negate the number, or `and` it with `2147483647` to drop the sign bit
- ⚠️ `bit_count` and `bit_length` now read a negative number as its 32 stored bits, as `and` and `not` do: both give 32 for `-1`
- ✨ Added `bs.bitwise:shift_left` and `bs.bitwise:shift_right`

...
assert run function #bs.collection:index_of {run: "execute if data storage bs:lambda collection{value: 3}"}
assert data storage bs:out {collection: {value: 2}}

# Find index of element with value 5 - 5 is at index 5
data modify storage bs:out collection.value set value [1, 2, 3, 4, 3, 5]
assert run function #bs.collection:index_of {run: "execute if data storage bs:lambda collection{value: 5}"}
...
assert run function #bs.collection:index_of {run: "execute if data storage bs:lambda collection{value: 'b'}"}
assert data storage bs:out {collection: {value: 1}}
...
assert run function #bs.collection:index_of {run: "execute if data storage bs:lambda collection{value: 42}"}

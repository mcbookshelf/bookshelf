...
function bs.collection:generate/call with storage bs:data collection.stack[-1]
...
execute store result score #l bs.ctx run data get storage bs:data collection.stack[-1].limit

# Recurse if limit > index
execute if score #l bs.ctx > #i bs.ctx run function bs.collection:generate/generate_rec

...
# Call the predicate function and return early if it succeed
execute store success score #s bs.ctx run function bs.collection:any/call with storage bs:data collection.stack[-1]
...
execute if data storage bs:data collection.stack[-1].value[0] run return run function bs.collection:any/any_rec
return fail

...
execute unless data storage bs:out collection.value[0] run return run function bs.collection:find_last/fail

$data modify storage bs:data collection.stack append value { run: "$(run)" }

execute store result storage bs:data collection.stack[-1].i int 1 run data get storage bs:out collection.value

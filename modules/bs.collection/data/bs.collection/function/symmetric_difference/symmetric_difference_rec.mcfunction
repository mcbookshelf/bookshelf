...
execute store success score #s bs.ctx run function bs.collection:utils/contains
execute if score #s bs.ctx matches 0 run data modify storage bs:out collection.value append from storage bs:ctx _.value[0]
execute if score #s bs.ctx matches 1 run data modify storage bs:ctx _.sym append from storage bs:ctx _.value[0]
